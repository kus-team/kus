...
app = FastAPI(
    title="KUS — Kill Unlawful Schemes",
    description=(
        "Антикоррупционная аналитика государственных закупок Узбекистана. "
        "Открытые данные с `data.egov.uz`, риск-скоринг по 3 ключевым флагам + 7 доп. сигналам, "
        "AI-нарратив через Claude Sonnet, профили компаний, граф связей, карта рисков по регионам. "
        "Публичный API без авторизации (rate-limit мягкий). "
        "Репо: https://github.com/kus-team/kus"
    ),
    version="0.2.0",
    contact={"name": "KUS Team", "url": "https://github.com/kus-team/kus"},
...
def audit(action: str, target: str | None = None, payload: dict | None = None, request: Request | None = None) -> None:
    ...
    try:
        ...
        try:
            cur = con.cursor()
            cur.execute(
                "INSERT INTO audit_log (action, target, ip, user_agent, payload) VALUES (%(a)s, %(t)s, %(i)s, %(u)s, %(p)s)",
                {"a": action, "t": target, "i": ip, "u": ua, "p": _json.dumps(payload or {}, ensure_ascii=False)},
            )
            con.commit()
        finally:
            con.close()
